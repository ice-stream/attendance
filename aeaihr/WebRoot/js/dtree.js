...
dTree.prototype.node = function(node, nodeId) {
	...
	if (node.url) {
		...
		str += '<a id="s' + this.obj + nodeId + '" class="' + ((this.config.useSelection) ? ((node._is ? 'nodeSel' : 'node')) : 'node') + '" oncontextmenu="' + node.url;
		if (this.config.useSelection && ((node._hc && this.config.folderLinks) || !node._hc)){
			str = str + ';' + this.obj + '.s(' + nodeId + ');"'+ '"' + '" href="' + node.url + '"';
		}else{
			str = str+ '"' + '" href="' + node.url + '"';	
		}
